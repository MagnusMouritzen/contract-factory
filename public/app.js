const requests = [
  ...
  },
  {
    ...
    method: "POST",
    description: "Successful POST user",
    endpoint: "/users",
...
function buildPact() {
  ...
  const interactions = requests.map(request => {
    ...
    if (headers && (Array.isArray(headers) || typeof headers !== "object")) errors.push(`${request.title}: headers must be a JSON object`);
  ...
  try {
    const res = await fetch("/api/publish", {
      ...
      body: JSON.stringify({
        pact,
        providerName,
        consumerVersion: el("consumerVersion").value
      })
    });
